...
const getApplicants = async(req, res) => {                //async makes a function return a Promise.
  try {                                                   //A JavaScript Promise object can be:
                                                          //Pending
                                                          // Fulfilled
                                                          // Rejected
    ...
    const data = await Applicants.findAll({               //await makes a function wait for a Promise.
      order: [['createdAt', 'DESC']],   //specifies the order according to createdAt Here it is descending order.
      attributes: ['applicantID', 
      ['name', 'candidate'], 
      [Sequelize.literal(`CASE WHEN isFresher = true THEN 'Fresher' ELSE 'Experienced' END`), 'type'], //Here, Sequelize.literal is SELECT *
